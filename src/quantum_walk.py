...
class QuantumWalk():
    ...
    def evolve(self):
        ...
        for node in self.__graph.nodes:
            new_state = self.get_new_state(node, self.__graph.xi)           
    ...
    def get_new_state(self, node, xi):
        ...
        # source
            ...
            return [(edge, source_val) for edge in node.outedges if edge.data['edge_type'] in ['v+','v-']] + [(edge, 0) for edge in node.outedges if edge.data['edge_type'] in ['h+','h-']]
        ...
        # dressed_photon
            ...
            coin_cls = Coin(node)
            coin = coin_cls.get(coin_name='dressed_photon', xi=xi, theta=node.data['coin_theta'])
    ...
    def to_plot(self, file_name):
        ...
        if self.step.step in [1,5000,5001,9000,9001,10000]:
...
class Coin():
    ...
    def get(self, coin_name, xi, theta):
        return self.dressed_photon(xi, theta)
    ...
    def dressed_photon(self, xi, theta):

        amp = np.cos(xi) + np.sin(xi)*1j
        ...
        coin = amp * np.vstack([up_coin, down_coin])
        return coin
