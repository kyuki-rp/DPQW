...
class Network():
    ...
    def custom(self, node_identifiers, sink=[], source=[], source_cycles={}, node_thetas={}):
        for node_identifier in node_identifiers:
        ...
        properties = dict([(si, 'sink') for si in sink] + [(so, 'source') for so in source])
    ...
    def get_edge_info(self, node_identifiers, sink, source):
        edges_info = []
        ...
        edges_info_ = []
        for (from_node_identifier, to_node_identifier, edge_type) in edges_info:
            if not (from_node_identifier in sink+source and to_node_identifier in sink+source):
                edges_info_.append((from_node_identifier, to_node_identifier, edge_type))        

        edges_info = edges_info_
